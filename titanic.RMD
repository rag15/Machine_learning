...
gs$Survived <- as.factor(gs$Survived)

cm <- confusionMatrix(myResults$Survived,gs$Survived)

myresults1 <- myResults[,c(1,12)]

# writing the results back into the file
write.csv(myResults,file = "myResults10.csv")
write.csv(myresults1,file = "myResults1.csv")
colnames(myResults)
q <- read.csv("myResults1.csv")
colnames(q)
q <- q[,-1]

write.csv(q,file = "myResults11.csv",row.names = F)
